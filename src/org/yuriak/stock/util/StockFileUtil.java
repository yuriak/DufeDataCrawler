...
public class StockFileUtil {
	public static void writeInfoToFile(ArrayList<StockBean> stocks) throws Exception{
		...
		FileUtils.writeStringToFile(new File(System.getProperty("user.dir")+File.separator+"data/stock"+time+".txt"), mainData.toString(), "UTF-8",false);
	}
	...
	public static ArrayList<StockBean> readInfoFromFile(String fileName){
		...
		for (int i = 0; i < stockArray.length(); i++) {
			...
			bean.setDate(StockTimeUtil.convertStringDateToDate(object.getString("date"),false));
			stockList.add(bean);
		}
		return stockList;
	}
